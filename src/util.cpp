...
const char * const BITCOIN_CONF_FILENAME = "dngrcoin.conf";
const char * const BITCOIN_PID_FILENAME = "dngrcoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "dngrcoin" is a composite category enabling all DNGRcoin-related debug output
            if(ptrCategory->count(string("dngrcoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\DNGRcoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\DNGRcoinCore
    // Mac: ~/Library/Application Support/DNGRcoinCore
    // Unix: ~/.dngrcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "DNGRcoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/DNGRcoinCore";
#else
    // Unix
    return pathRet / ".dngrcoin";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty dngrcoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override dngrcoin.conf
