...
*Setup instructions for a Gitian build of DNGRcoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the DNGRcoin
...
- [Building DNGRcoin](#building-dngrcoin-core)
...
Clone the git repositories for DNGRcoin and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/dngrcoincoin/dngrcoin
```
...
in the DNGRcoin repository under 'Fetch and build inputs' to install sources which require
...
To build DNGRcoin (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the DNGRcoin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/dngrcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/dngrcoincoin/dngrcoin
...
For example:
```bash
URL=https://github.com/crowning-/dngrcoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit dngrcoin=${COMMIT} --url dngrcoin=${URL} ../dngrcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit dngrcoin=${COMMIT} --url dngrcoin=${URL} ../dngrcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit dngrcoin=${COMMIT} --url dngrcoin=${URL} ../dngrcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the dngrcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../dngrcoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/dngrcoin.git
SIGPATH=/some/root/path/dngrcoin-detached-sigs.git

./bin/gbuild --url dngrcoin=${BTCPATH},signature=${SIGPATH} ../dngrcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/dngrcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/dngrcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/dngrcoin-osx-build.assert
```
...
[dngrcoin/gitian.sigs](https://github.com/dngrcoincoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
